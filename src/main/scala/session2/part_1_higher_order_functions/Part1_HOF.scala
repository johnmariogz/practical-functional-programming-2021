package berlin.code.functional
package session2.part_1_higher_order_functions

object Part1_HOF extends App {
  val list = List(0, 1, 2, 3, 4, 6, 7, 8, 9)

  val byTwo: List[Int] = ??? // TODO Make it multiply all by two
  println(s"All values multiplied by two are = $byTwo")

  val onlyOdd: List[Int] = ??? // TODO Make it keep only ODD numbers
  println(s"The odd numbers in the list are = $onlyOdd")

  val totalSum: Int = ??? // TODO Make it sum all elements
  println(s"The sum of all elements is = $totalSum")
}
